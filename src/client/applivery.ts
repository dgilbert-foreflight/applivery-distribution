...
import type {
  Build,
  BuildUploadResponseRaw,
  CreatePublicationRequest,
  Publication,
  PublicationCreateResponseRaw,
  PublicationQueryParams,
  PublicationsResponseRaw,
  UploadBuildRequest
} from './models.js'
...
export async function fetchPublications(
  apiKey: string,
  baseUrl: string,
  queryParams?: PublicationQueryParams,
  ...
  const buildQueryString = (
    page: number,
    customParams?: PublicationQueryParams
  ): string => {
  ...
  try {
    do {
      ...
      core.debug(
        styles.color.cyan.open + `Request URL: ${url}` + styles.color.cyan.close
      )

      let response: AxiosResponse<PublicationsResponseRaw>
      try {
        ...
        if (axios.isAxiosError(error)) {
          if (error.response) {
            ...
            throw new AppliveryAPIError(
              `Failed to fetch publications: ${error.response.statusText || 'Unknown error'}`,
              error.response.status,
              JSON.stringify(error.response.data)
            )
          } else if (error.request) {
            // Network error
            throw new AppliveryNetworkError(
              `Network error while fetching publications from ${url}`,
              error
            )
          } else {
            // Other error
            throw new AppliveryNetworkError(
              `Error while fetching publications from ${url}`,
              error
            )
          }
        } else {
          throw new AppliveryNetworkError(
            `Unexpected error while fetching publications from ${url}`,
            error as Error
          )
        }
      }
      ...
      switch (response.status) {
        ...
        default:
          ...
          throw new AppliveryAPIError(
            `Failed to fetch publications: ${response.statusText || 'Unknown error'}`,
            response.status,
      ...
      if (!response.data.data || !Array.isArray(response.data.data.items)) {
        throw new AppliveryParseError(
          'Invalid response structure: missing data.items array',
          JSON.stringify(response.data)
        )
      }

      // Transform raw publications to have proper Date objects
      let transformedItems = response.data.data.items.map(
        publicationTransformer
      )
      ...
      if (queryParams?.filterType && queryParams?.filterValue) {
        transformedItems = transformedItems.filter((item) => {
          return (
            item.filter.type === queryParams.filterType &&
      ...
      core.debug(
        `Fetched ${transformedItems.length} items from page ${currentPage}/${totalPages}`
      )
      currentPage++
      core.endGroup()
    ...
    core.debug(
      styles.cyan.open + JSON.stringify(allItems, null, 2) + styles.cyan.close
    )
    core.endGroup()
    return allItems
  } catch (error) {
    ...
    if (
      error instanceof AppliveryAPIError ||
...
export async function createPublication(
  ...
  let response: AxiosResponse<PublicationCreateResponseRaw>
  try {
    ...
    if (axios.isAxiosError(error)) {
      if (error.response) {
        // Server responded with error status
        throw new AppliveryAPIError(
          `Failed to create publication: ${error.response.statusText || 'Unknown error'}`,
          error.response.status,
          JSON.stringify(error.response.data)
        )
      } else if (error.request) {
        // Network error
        throw new AppliveryNetworkError(
          `Network error while creating publication at ${url}`,
          error
        )
      } else {
        // Other error
        throw new AppliveryNetworkError(
          `Error while creating publication at ${url}`,
          error
        )
      }
    } else {
      throw new AppliveryNetworkError(
        `Unexpected error while creating publication at ${url}`,
        error as Error
      )
    }
  }
  ...
  switch (response.status) {
    ...
    default:
      ...
      throw new AppliveryAPIError(
        `Failed to create publication: ${response.statusText || 'Unknown error'}`,
        response.status,
        JSON.stringify(response.data)
      )
  }
...
export async function uploadBuild(
  ...
  if (payload.notifyCollaborators)
    formData.append(
      'notifyCollaborators',
      payload.notifyCollaborators.toString()
    )
  ...
  if (payload.deployer) {
    ...
    if (payload.deployer.info) {
      ...
      if (info.commit) formData.append('deployer.info.commit', info.commit)
      if (info.branch) formData.append('deployer.info.branch', info.branch)
      if (info.tag) formData.append('deployer.info.tag', info.tag)
      ...
      if (info.ciUrl) formData.append('deployer.info.ciUrl', info.ciUrl)
  ...
  let response: AxiosResponse<BuildUploadResponseRaw>
  try {
    ...
    if (axios.isAxiosError(error)) {
      if (error.response) {
        // Server responded with error status
        throw new AppliveryAPIError(
          `Failed to upload build: ${error.response.statusText || 'Unknown error'}`,
          error.response.status,
          JSON.stringify(error.response.data)
        )
      } else if (error.request) {
        // Network error
        throw new AppliveryNetworkError(
          `Network error while uploading build to ${url}`,
          error
        )
      } else {
        // Other error
        throw new AppliveryNetworkError(
          `Error while uploading build to ${url}`,
          error
        )
      }
    } else {
      throw new AppliveryNetworkError(
        `Unexpected error while uploading build to ${url}`,
        error as Error
      )
    }
  }
  ...
  switch (response.status) {
    ...
    default:
      throw new AppliveryAPIError(
        `Failed to upload build: ${response.statusText || 'Unknown error'}`,
        response.status,
        JSON.stringify(response.data)
      )
  }
...
export async function getBuild(
  ...
  let response: AxiosResponse<Build>
  try {
    response = await axios.request(config)
  } catch (error) {
    if (axios.isAxiosError(error)) {
